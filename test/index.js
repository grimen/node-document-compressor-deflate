...
module.exports = {
  ...
  'Deflate.prototype': {
    ...
    '#engine': function() {
      ...
      assert.equal ( compressor.engine, require('compress-buffer') );
    },
    ...
    '#compress': {
      ...
      '(uncompressed string)': function() {
        ...
        assert.deepEqual ( compressor.compress(input), require('compress-buffer').compress(new Buffer(input)) );
      },

      '(uncompressed object)': function() {
        var input = {_id: 1, a: "foo", b: "bar"};

        assert.deepEqual ( compressor.compress(input), require('compress-buffer').compress(new Buffer(JSON.stringify(input))) );
      }
    },
    ...
    '#decompress': {
      ...
      '(compressed string)': function() {
        var input = JSON.stringify({_id: 1, a: "foo", b: "bar"});
        var compressed = require('compress-buffer').compress(new Buffer(input));

        assert.deepEqual ( compressor.decompress(compressed), require('compress-buffer').uncompress(compressed) );
      },

      '(compressed object)': function() {
        var input = {_id: 1, a: "foo", b: "bar"};
        var compressed = require('compress-buffer').compress(new Buffer(JSON.stringify(input)));
